...
int main(int argc, char** argv) try
{
	...
	filesys::path pathObj(inputFilename.getValue());
	std::string tagFile = pathObj.stem().string();
	std::string tagFileTime = pathObj.parent_path().string() + "\\" + tagFile + ".txt";
	std::string tagFileFrame = pathObj.parent_path().string() + "\\" + tagFile + ".dat";

	string iss;
	ifstream infile(tagFileTime.c_str());
	vector < string > v;
	std::map < string, unsigned long long> tagTime,tagFrame;

	if (infile.fail())
	{
		cout << "\n\tThe file named " << tagFileTime << " was NOT successfully opened."
	...
	{
		unsigned long long li_dec = std::stoull(v[i + 1],nullptr, 16);
		tagTime.insert({v[i], li_dec});
	...
	while (pipe->try_wait_for_frames(&frameset, 1000))
	{
		...
		for_each(converters.begin(), converters.end(),
			[&frameset, &pos](shared_ptr<rs2::tools::converter::converter_base>& converter) {
				converter->convert(frameset, pos);
			});
		...
		uint64_t li_dec = std::stoull(v[index + 1]);
		if ((static_cast<int64_t>(li_dec - posLast) > 0 && static_cast<int64_t>(li_dec - posCurr) < 0) || static_cast<int64_t>(li_dec - posCurr) == 0) {
			tagFrame.insert({ v[index], pos });
			if((index+2)<v.size())
			index += 2;
		}
	...
	for (auto& tag : tagFrame)		
		{
			content << tag.first << "," << tag.second << ",";
		}
